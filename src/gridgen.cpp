...
/***********************************************************************
void gridgen::orbs_overlap(){
	qm_calc->set_grid_dim(Origin,grid_sides);
	omp_set_num_threads(NP);
	#pragma omp parallel for collapse(3)
	for (int x=0;x<grid_len[0];x++){
		for (int y=0;y<grid_len[1];y++){
			for (int z=0;z<grid_len[2];z++) {
				psi[x][y][z] = qm_calc->overlap_density(x,y,z);
			}
		}
	}
	molecule.reset(new Imolecule (*qm_calc->molecule) );
	density->add_data(psi,Origin,grid_len,grid_sides,*molecule);
	density->name = name;
}
/***********************************************************************/
gridgen::~gridgen(){}
