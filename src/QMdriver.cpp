...
double QMdriver::EAS_in_atoms(int atom,int band){
	...
	if ( molecule.m_overlap.size() <= 0 ) {
		cout << "Error: There are no overlap matrix elements stored! \n "
				<< "Review your Fukui method for condensed descriptors or your provided QM output" << endl;
		exit(-1);
	}
	...
	for( int i=init;i<=homon;i++){
		...
		if ( molecule.betad ){
			...
			if ( atom == 0  && band > 0 ){
				m_log->input_message("Number of occcupied beta MO used to calculate condensed to atom descriptors: ");
				m_log->input_message(cnt);
			}
			value /= 2;
		}
