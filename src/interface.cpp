...
interface::interface(int argc, char** argv):
	...
	if ( m_argc >=3 )  {
		file_name1 = m_argv[2];
		if ( file_name1 == "-log") {
			file_name1 = "/home/igorchem/debug.log";
		}else{	file_name1 += ".log"; }
	}
	m_log->initialize(file_name1,M_verbose,M_logfile);
			...
			if ( check_file_ext(".aux",fnames[i].c_str()) ){
				...
				if ( option == 2 ) {
					inp_file << option	<< " " << fnames[i] 
								<< " "		<<	change_extension(fnames[i].c_str(),"_cat.aux")	<< " "
								<< " "		<<	change_extension(fnames[i].c_str(),"_an.aux")	<< " "
