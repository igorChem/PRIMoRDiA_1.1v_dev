		...
		for(i=0;i<buffer->nLines;i++){
			if ( i ==0 ) {
				...
				if ( buffer->lines[i].IF_word("HEAT_OF_FORMATION:",0,18)) {
					...
					}else if ( buffer->lines[i].IF_word("TOTAL_ENERGY:EV",0,15) ){
						string word1 ( buffer->lines[i].words[0],16,14);
...
void mopac_files::parse_mgf(){
	...
	for(unsigned int i=0;i<molecule->atoms.size();i++) {
		molecule->mol_charge	+= molecule->atoms[i].charge;
		molecule->molar_mass	+= molecule->atoms[i].atomic_mass;
	}
	
	if ( molecule->num_of_electrons == 0 ){
		...
		if ( molecule->occupied_beta.size() > 0 ){
			molecule->num_of_electrons += molecule->occupied_beta[i];
		}
	}
	molecule->get_ao_number();
	molecule->get_homo();
