...
void primordia::init_FOA(const char* file_neutro	,
	...
	if ( gridn > 0 ){
		unique_ptr<gridgen> grid1( new gridgen(gridn,move(lrdCnd->molecule) ) );
	...
	if ( pymol_script ) { 
		//Iprotein mol;
		//string namepdb = name + ".pdb";
		//Imolecule mol1 = *lrdCnd->molecule;
		//mol.write(namepdb,*lrdCnd->molecule);
...
void primordia::init_FD(const char* file_neutro,
	...
	if ( gridn > 0 ){
		unique_ptr<gridgen> grid1 ( new gridgen( gridn,move(molecule_a) ) );
		if ( program == "orca" ) grid1->calculate_density_orca();
		else {	grid1->calculate_density();	}
	 
		unique_ptr<gridgen> grid2 ( new gridgen( gridn,move(molecule_b) ) );
		if ( program == "orca" ) grid2->calculate_density_orca();
		else { grid2->calculate_density();	}
	
		unique_ptr<gridgen> grid3 ( new gridgen( gridn, move(molecule_c) ) );
...
void primordia::init_protein_RD(const char* file_name	,
	...
	Imolecule molecule ( fileQM->get_molecule(band2)  );
	...
	if ( gridn > 0 ){
		...
		if ( size > 0 ) { grid->redefine_lim(ref_atom[0],ref_atom[1],ref_atom[2],size); }
		Icube EAS;
		Icube NAS;
		if ( band  > 0 ) {
			if ( bt == "BD" ){
